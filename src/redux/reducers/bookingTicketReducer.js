...
export const bookingTicketReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case XAC_NHAN:
          let mangGhe = [];
          for(const hangGhe of state.mangGhe){
            for(const ghe of hangGhe.danhSachGhe){
                mangGhe.push(ghe)
            }
          }
          state.mangGheDangDat.map((gheDangDat) => { 
            let gheTK = mangGhe.find(ghe => ghe.soGhe == gheDangDat.soGhe)
            gheTK.daDat = true;
           })
